...
$dotaMonitorJob = Start-PythonJob -scriptPath "dota_monitor.py" -jobName "DotaMonitorJob"
...
Stop-Job -Id $dotaMonitorJob.Id

Start-Sleep -Seconds 2

$serverJobState = (Get-Job -Id $serverJob.Id).State
$senderJobState = (Get-Job -Id $senderJob.Id).State
$dotaMonitorJobState = (Get-Job -Id $dotaMonitorJob.Id).State

if ($serverJobState -eq "Running" -or $senderJobState -eq "Running" -or $dotaMonitorJobState -eq "Running") {
...
Remove-Job -Id $dotaMonitorJob.Id -ErrorAction SilentlyContinue
